<?php
  ...
  function image_upload($datafile)
  {
	...
	if ($_REQUEST['action'] == 'cancel_row') 
	{
		...
		$delete = mysqli_query($connect, "UPDATE `member` SET is_canceled=1 WHERE id='$row_id'");
		if (!$delete) 
		{	
			$return_data["head"]="error";
    ...
    if(!empty($row_id))
    {
      $m_query = mysqli_query($connect, "SELECT user_id FROM member WHERE id='$row_id' ");
